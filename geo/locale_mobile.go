...
// locale_mobile.go —— 国家码 → Android 下划线 locale。一张静态表 + 一个查表函数。
// 文件名不能用 locale_android.go：Go 会把 *_android.go 当成 GOOS=android，本机直接忽略。
// 与 Web 表分开维护；keyset 由 TestTimezoneTableKeysetMatchesLocaleTable 对齐。

// countryToMobileLocale 国家码 → Android 下划线 locale，与 Web 表无关。
// JP 是 ja_JP，不会因为 Web 收成 ja 而变成 ja。
var countryToMobileLocale = map[string]string{
	...
	// 东亚
	"CN": "zh_CN", "TW": "zh_TW", "HK": "zh_HK", "MO": "zh_HK",
	...
	"GR": "el_GR", "CY": "el_CY", "HR": "hr_HR", "RS": "sr_RS", "SI": "sl_SI",
	"BA": "bs_BA", "MK": "mk_MK", "AL": "sq_AL", "ME": "sr_ME", "XK": "sq_XK",
...
// MobileLocaleForCountry 按国家代码查 Android 下划线 locale。
// 输入 country：ISO 3166-1 alpha-2，大小写不敏感，自动 trim。
// 返回：表值（如 ja_JP）；空输入或未命中返回 ("", *UnknownCountryError)。
